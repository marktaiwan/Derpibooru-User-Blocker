...
(function () {
  ...
  config.registerSetting({
    title: 'Block staff',
    key: 'block_staff',
    description: 'Spare no quarter. Block even the site staff.',
    type: 'checkbox',
    defaultValue: false
  });
  config.registerSetting({
  ...
  const BLOCK_STAFF = config.getEntry('block_staff');
...
`;
  ...
  */
  ...
  function isStaff(comment) {
    const nodeList = comment.querySelectorAll('.label--danger, .label--success, .label--purple');
    return (nodeList.length > 0);
  }

  function hideComment(comment) {
    if (isStaff(comment) && !BLOCK_STAFF) return;
    if (STUB) {
